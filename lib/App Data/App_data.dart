...
// ########################################### Api Keys ##################################
  ...
  static Future<List<dynamic>?> checkPaymentApiStatus(String domainUrl) async {
  final url = Uri.parse(
  ( domainUrl),
  );
  final request = http.Request('GET', url);
  ...
  const Duration(seconds: timer),
  onTimeout: () {
  throw TimeoutException("Payment API request timed out");
  },
  );
  ...
  if (kDebugMode) {
  print('Payment API request failed: ${response.statusCode} - ${response.reasonPhrase}');
  }
  }
  } on TimeoutException catch (_) {
  if (kDebugMode) {
  print("Payment API request exceeded $timer seconds — marking as DOWN");
  }
  return [
  {
  "component": "PaymentApiStag",
  "status": "DOWN",
  "message": "Timeout after $timer seconds",
  "timestamp": DateTime.now().toIso8601String(),
  }
  ...
  if (kDebugMode) {
  print('Payment API request exception: $e');
  }
  }
